...
每个阶段交付前，Codex 必须完成其权限范围内的全部实现、测试、提交和可验证证据；不得把可由 Codex 完成的工作转交给铭哥。ChatGPT 任务由 Codex 提供可直接粘贴的提示词、固定输入链接和预期输出；铭哥只负责外部授权、必须的人机验证、产品判断和阅读验收。具体交接格式见 `docs/operations/runbook.md`。
...
进入 G3 前，必须通过 G2 只读验收：四档窗口应分别控制召回；未知事件时间不得伪装为文章发布时间；流水线必须能产生 `success`、`partial`、`failed`；`recent-events.json` 必须来自此前成功候选包的跨批次交接记录。
